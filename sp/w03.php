<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=320px, user-scalable=no, initial-scale=1, maximum-scale=1" />
<title>遠鉄アシスト｜仕事を探す</title>
<?php include 'include/meta.inc'; ?>

</head>
<body>
<div id="view">
<div class="wrap">
<?php include 'include/header.inc'; ?>

<div id="contents">
<article>
<h2 class="stitle"><img src="imgs/title_works.png" alt="検索結果" width="100%"></h2>
<section>
<div class="worksDetail">
    <div class="workData">
        <div class="workName">クリーンスタッフ</div>
        <ul class="workPoint">
        	<li>時給: 860円</li>
        	<li>7:30~11:30</li>
        </ul>
        <dl class="workInfo">
        	<dt>仕事の内容</dt>
            <dd>事務所・工場内の日常清掃（玄関・通路・トイレ等）</dd>
            
            <dt>勤務地</dt>
            <dd>事務所・工場ビル（東区常光町）</dd>
            
            <dt>時間</dt>
            <dd>5:30~8:30</dd>
            
            <dt>休憩時間</dt>
            <dd></dd>
            
            <dt>給与</dt>
            <dd>時給820~850円　※見習期間3ヶ月は、時給-10円</dd>
            
            <dt>日数</dt>
            <dd>週3日程度（シフト制）</dd>
            
            <dt>休日</dt>
            <dd>土日祝他（工場カレンダーによる）</dd>
            
            <dt>雇用形態</dt>
            <dd>パート</dd>
            
            <dt>雇用期間</dt>
            <dd></dd>
            
            <dt>学歴</dt>
            <dd></dd>
            
            <dt>職場の雰囲気</dt>
            <dd>女性活躍中　未経験者歓迎　丁寧な研修あり♪</dd>
            
            <dt>資格</dt>
            <dd></dd>
            
            <dt>備考</dt>
            <dd>交通費規定支給、制服貸与、車通勤可</dd>
        </dl>
        
        <a class="workLogin">ログイン</a>
        
        <p class="textOR">もしくは・・・</p>
        
        <div class="workEntryBox">
            <form action="">
            <dl>
            	<dt>名前</dt>
                <dd><input type="text"></dd>
            	<dt>フリガナ</dt>
                <dd><input type="text"></dd>
            	<dt>電話番号</dt>
                <dd><input type="text"></dd>
            	<dt>メールアドレス</dt>
                <dd><input type="text"></dd>
            </dl>
            
            <a href="" class="workEntryBtn">担当者からすぐ連絡希望</a>
            <p class="textOR">もしくは・・・</p>
            <a href="" class="workEntryBtn">応募する</a>
            </form>
        </div>
        
        <div class="worksBtnBox">
        	<a class="btnOther" href="works.php">似た仕事を検索</a>
        	<a class="btnBack" href="works.php">戻る</a>
        </div>
    </div>
</div>
</section>

</article>
</div><!-- /.contents -->

<?php include 'include/footer.inc'; ?>

</div><!-- /.wrap -->
</div><!-- /#view -->

<?php include 'include/js.inc'; ?>


</body>
</html>
